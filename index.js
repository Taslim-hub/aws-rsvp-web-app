...
export const handler = async (event) => {
  ...
  try {
    ...
    */
    if (method === "GET" && path.startsWith("/stats/")) {
      ...
      const result = await dynamo.send(new BatchGetItemCommand({
        RequestItems: { "event-rsvp-responses": { Keys: keys } } // Replace "event-rsvp-counts" with your own DynamoDB table name
      }));

      const items = result.Responses?.["event-rsvp-responses"] || []; // Replace "event-rsvp-counts" with your own DynamoDB table name
    ...
    */
    if (method === "POST" && path.includes("/rsvp")) {
      ...
      try {
        await dynamo.send(new TransactWriteItemsCommand({
          TransactItems: [
            {
              Put: {
                TableName: "event-rsvp-responses", // Replace "event-rsvp-counts" with your own DynamoDB table name
                Item: {
                  pk:        { S: `EVENT#${event_id}` },
